...
MOD_EXPORT ModResult mod_initialize(ModError*) {
    ...
    result = svc_hook->add_post<SetDaytime>(mod_ctx, on_set_daytime_post);
    if (result != MOD_OK) {
    ...
    result = svc_hook->add_post<ActPerformance>(mod_ctx, on_act_performance_post);
    if (result != MOD_OK) {
        svc_log->error(mod_ctx, "failed to install on_act_performance_post");
        return result;
    }

    result = svc_hook->add_pre<ActPerformance>(mod_ctx, on_act_performance_pre);
    if (result != MOD_OK) {
        svc_log->error(mod_ctx, "failed to install on_act_performance_pre");
        return result;
    }

    result = svc_hook->add_pre<InstantTimechg>(mod_ctx, on_instant_timechg_pre);
    if (result != MOD_OK) {
        svc_log->error(mod_ctx, "failed to install on_instant_timechg_pre");
        return result;
    }

    result = svc_hook->add_post<KankyoCreate>(mod_ctx, on_kankyo_create_post);
    if (result != MOD_OK) {
    ...
    result = svc_hook->add_post<Kytag11Execute>(mod_ctx, on_kytag11_execute_post);
    if (result != MOD_OK) {
        svc_log->error(mod_ctx, "failed to install on_kytag11_execute_post");
        return result;
    }

    result = svc_hook->add_pre<Kytag11Execute>(mod_ctx, on_kytag11_execute_pre);
    if (result != MOD_OK) {
